...
if (-not (Test-Path $paths.IMPL_PLAN -PathType Leaf)) {
    ...
    [Console]::Error.WriteLine("Run /vipd-speckit-plan first to create the implementation plan.")
    exit 1
}

if (-not (Test-Path $paths.FEATURE_SPEC -PathType Leaf)) {
    [Console]::Error.WriteLine("ERROR: spec.md not found in $($paths.FEATURE_DIR)")
    [Console]::Error.WriteLine("Run /vipd-speckit-specify first to create the feature structure.")
    exit 1
}
