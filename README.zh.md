...
![vibe-ipd 架构概览](./architect-view.png)

## 功能目录

vibe-ipd v1.0.0 实现了 **17 项功能**，覆盖完整的 IPD 工作流：

| # | 功能 | 状态 |
|---|------|------|
| 001 | IPD 工具包 — 核心门禁框架与章程 | ✅ |
| 002 | IPD 智能体 PM 集成 | ✅ |
| 003 | 蓝图纯文档状态模式 | ✅ |
| 004 | VIPD 命令前缀迁移 | ✅ |
| 005 | 实现 IPD 门禁（TR0-TR6） | ✅ |
| 006 | 文档状态工具 | ✅ |
| 007 | PDT 角色设置与 RACI 矩阵 | ✅ |
| 008 | CLI IPD 适配 | ✅ |
| 009 | IPD 迁移差距填补（25项） | ✅ |
| 010 | 端到端工作流验证 | ✅ |
| 011 | 多语言问候（--lang en/zh/ja） | ✅ |
| 012 | 项目清理与文档重塑 | ✅ |
| 013 | VIPD 初始化技能（uvx/pipx） | ✅ |
| 014 | 中文文档翻译 | ✅ |
| 015 | 修复 TR 门禁循环依赖 | ✅ |
| 016 | VIPD 语言选项（--lang 标志） | ✅ |
| 017 | **版本管理与文档准备** | ✅ **v1.0.0** |

> 运行 `vipd --version` 查看当前版本和底层 speckit 版本。

## 版本管理

vipd 遵循**语义化版本控制**（MAJOR.MINOR.PATCH），规则与 speckit 相同：

| 升级类型 | 何时使用 |
|----------|----------|
| **MAJOR** | 破坏性工作流变更、不兼容的格式变更 |
| **MINOR** | 新增功能（完成新 spec） |
| **PATCH** | 错误修复、文档更新、重构 |

vipd 版本与 speckit 版本独立跟踪。运行 `vipd --version` 查看两者：

```
vipd 1.0.0 (speckit 0.9.3.dev0)
```

详见 [VERSION_BUMP.md](./VERSION_BUMP.md) 了解完整升级流程，[CHANGELOG.md](./CHANGELOG.md) 查看发布历史。
...
**当前版本**：`vipd 1.0.0`（speckit 0.9.3.dev0）— 首个正式版发布！🎉
