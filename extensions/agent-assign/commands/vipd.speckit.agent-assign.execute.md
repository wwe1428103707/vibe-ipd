...
2. **Check Assignment File Exists**: Verify `agent-assignments.yml` exists in FEATURE_DIR.
   - If missing, **STOP** and report: "No agent-assignments.yml found. Run `/vipd.speckit.agent-assign.assign` first, then `/vipd.speckit.agent-assign.validate` to verify."
...
4. **Project Setup Verification** (same as `/vipd.speckit.implement`):
...
6. **Execute Tasks Phase by Phase**: For each phase in tasks.md, process tasks in order:
   ...
   **Mode A — Default (no specialized agent)**:
   If the task is assigned to `default`, execute the task directly in the current context, following the same implementation rules as `/vipd.speckit.implement`:
...
Note: This command requires both `tasks.md` and `agent-assignments.yml` to exist. If tasks.md is missing, suggest running `/vipd.speckit.tasks` first. If agent-assignments.yml is missing, suggest running `/vipd.speckit.agent-assign.assign` first.
