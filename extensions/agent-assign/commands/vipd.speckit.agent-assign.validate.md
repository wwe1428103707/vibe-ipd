...
handoffs:
  - label: Reassign Agents
    agent: vipd.speckit.agent-assign.assign
...
2. **Check Assignment File Exists**: Verify `agent-assignments.yml` exists in FEATURE_DIR.
   - If missing, **STOP** and report: "No agent-assignments.yml found. Run `/vipd.speckit.agent-assign.assign` first to generate agent assignments."
...
6. **Generate Validation Report**: Output a structured report:
   ...
   **Validation time**: <timestamp>
   ...
   - Run `/vipd.speckit.agent-assign.assign` to reassign tasks with missing or conflicting agents
   - Or manually edit `agent-assignments.yml` to fix specific entries
   ```

7. **Final Verdict**:
   - **PASS**: All checks pass — safe to run `/vipd.speckit.agent-assign.execute`
   - **FAIL**: Issues found — list actionable remediation steps

   If PASS, suggest proceeding with `/vipd.speckit.agent-assign.execute`.
   If FAIL, suggest running `/vipd.speckit.agent-assign.assign` to fix issues.
